...
for threshold in range(0, 2*int(max(distances.mean(axis=1)))):
    outlier_index = np.where(distances.mean(axis = 1) > threshold/2)[0]
    ...
    if MCC > highest_MCC:
        params = [threshold/2, accuracy, recall, precision, F1_score, MCC]
        highest_MCC = MCC
